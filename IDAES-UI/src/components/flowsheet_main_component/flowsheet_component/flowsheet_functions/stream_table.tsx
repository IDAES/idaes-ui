...
export class StreamTable {
    ...
    highlightFn: any;
    removeHighlightFn : any;
    gridCellMouseEnterFn : any;
  ...
  emptyVarTypesPanel () {
      this.existing_var_types = new Set();
      const var_types_panel:HTMLElement | null= document.querySelector('#existing-variable-types');
  ...
  fillVarTypesPanel() {
      ...
      // Adding each type
      this.existing_var_types.forEach((var_type:any) => {
          switch (var_type) {
              case this.UNFIXED:
              ...
              default:
                //   console.warn(`Couldn't identify Variable type: ${data[col_index]}`);
                  console.warn(`Couldn't identify Variable type`);
          };
      });
  }
    ...
    fillTable(model:any) {
            ...
            for (let col_index in columns) {
                if (columns[col_index] === "Units") {
                    if (data[col_index] && data[col_index].html) {
                        (row_object as any)[variable_col] = (row_object as any)[variable_col] + '<span class="streamtable-units">' + data[col_index].html + '</span>';
                    }
                    else {
                        (row_object as any)[variable_col] = (row_object as any)[variable_col] + '<span class="streamtable-units">&ndash;</span>';
                    }
                }
                else if (columns[col_index] === "Variable") {
                    (row_object as any)[columns[col_index]] = data[col_index];
                }
                else {
                    ...
                    (row_object as any)[columns[col_index]] = cell_style + '<span class="streamtable-variable-value">' + value + '</span>';
                }
            };
        ...
        this._gridOptions.getRowStyle = function(params:any) {
    ...
    defineTableBrushingFns() {
        ...
        this.gridCellMouseEnterFn = (event: MouseEvent) => {
            ...
            // if (document.querySelector("#view-stream-highlight-btn").checked) {
                ...
                if (colIdAttribute) {
                    const highlightStreamEvent = new CustomEvent('HighlightStream', {
                        detail: {
                            streamId: colIdAttribute
                        }
                    });
        ...
        this.gridCellMouseLeaveFn = (event: MouseEvent) => {

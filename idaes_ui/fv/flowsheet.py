...
class FlowsheetSerializer:
    ...
    def _clean_values(self, df: "DataFrame"):
        """
        Replacing NaN, Infinity, and Negative Infinity with strings to make valid
        JSON for front end consumption

        Args:
            df: The Pandas dataframe to replace NaN, Infinity, and Negative Infinity
                with strings
        Return:
            The dataframe that now has valid JSON
        """
        df = df.replace(math.nan, "NaN")
        df = df.replace(math.inf, "Inf")
        df = df.replace(-math.inf, "-Inf")
        return df
    ...
    def _add_unit_model_with_ports(self, unit, unit_type):
        ...
        if unit.parent_block() == self.flowsheet:
            ...
            if stream_df is not None and not stream_df.empty:
                # If there is a stream dataframe then we need to reset the index so we
                # can get the variable names and then rename the "index"
            ...
            for port in unit.component_objects(Port, descend_into=False):
                self.ports[port] = parent_unit
                # Add all of this subcomponent's ports to used_ports; realistically,
                # this is only relevant in a situation where the subcomponent has ports
                # that are not connected to an Arc that we intend to display.
                # Otherwise, the port is marked as "used" when we traverse the Arcs.
        ...
        for port in sorted(untouched_ports, key=lambda p: str(p)):
            ...
            if feed_match and product_match:
                self._logger.warning(
                    "Port looks like both a feed and a product: "
                ...
                try:
                    ...
                    self._logger.warning(
                        "Cannot extract state block from Port: "
                        f"name={port_name}. "
                        "Please add Feed & Product blocks with Arcs to show "
                        "inlet and outlet stream values in the Stream Table"
                    )
            else:
                self._logger.warning(
                    "Port is neither a feed nor a product: "
                    f"name={port_name} feed-expr={self.INLET_REGEX} "
                    f"product-expr={self.OUTLET_REGEX}"
                )
    ...
    def _construct_model_json(self):
        ...
        self._stream_table_df = self._clean_values(self._stream_table_df)
        # self._stream_table_df.where(
        ...
        # {'index': ["('Liq', 'benzene')", "('Liq', 'toluene')", "('Liq', 'hydrogen')",\
        # "('Liq', 'methane')", "('Vap', 'benzene')", "('Vap', 'toluene')",\
        # "('Vap', 'hydrogen')", "('Vap', 'methane')", 'temperature', 'pressure'],
        # 'columns': ['s03', 's04', 's05', 's06', 's08', 's09', 's10'], # 7 columns
        # 'data': [[0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5], ...]]}          # 7 values
        ...
        for unit_model in self.unit_models.values():
            ...
            def round_number(x, n=self._sig_figs):
                """Round, if a number, to 'n'; otherwise don't change."""
